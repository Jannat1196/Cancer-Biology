...
keep_stad <- rowSums(cpm(stad_counts) > 1) >= 0.1*ncol(stad_counts)
...
#STAD
stad_up   <- stad_degs[stad_degs$logFC >  2 & stad_degs$PValue < 0.01, ]
stad_down <- stad_degs[stad_degs$logFC < -2 & stad_degs$PValue < 0.01, ]
nrow(stad_down)
nrow(stad_up)
...
top100_up <- stad_degs_annot[
  stad_degs_annot$logFC > 2,
][order(stad_degs_annot$PValue[stad_degs_annot$logFC > 2]), ][1:100, ]
